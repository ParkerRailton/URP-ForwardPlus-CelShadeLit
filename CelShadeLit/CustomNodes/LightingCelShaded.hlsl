...
float3 CalculateCelShading(Light l, SurfaceVariables s, half shadowValue)
{
    float attenuation =
    ...
    return (l.color * (diffuse + max(specular, rim))) * shadowValue;
}
#endif
      ...
      float EdgeShadowAttenuation, float EdgeRim, bool CapColorValue, float CapThresh, out float3 Color)
{
   ...
   float4 shadowCoord = ComputeScreenPos(clipPos);
    ...
    Color = CalculateCelShading(light, s, 1);
...
LIGHT_LOOP_BEGIN(pixelLightCount)
    ...
    half shadowValue = AdditionalLightRealtimeShadow(lightIndex, Position);
    Color += CalculateCelShading(addLight, s, shadowValue);
LIGHT_LOOP_END

#endif
    if (CapColorValue)
    {
        float cMagnitude = length(Color);
        if (cMagnitude > CapThresh)
        {
            float cMod = CapThresh / cMagnitude;
            Color.x *= cMod;
            Color.y *= cMod;
            Color.z *= cMod;
        }
